...
func NewPostgresWithPool(ctx context.Context, databaseURL string, poolCfg DBPoolConfig) (*PostgresStore, error) {
	...
	// W19-27: run golang-migrate before any inline helpers. Migration files
	// are the canonical schema source of truth (Decision Log 2026-04-28).
	// Any error here is fatal — startup fails fast so Railway/os.Exit(1) fires.
	if err := runPostgresMigrations(db); err != nil {
		db.Close()
		return nil, fmt.Errorf("running postgres file migrations: %w", err)
	}
